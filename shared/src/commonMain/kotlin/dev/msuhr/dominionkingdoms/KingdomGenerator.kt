...
// Instantiated via Hilt @Provides in the Android app (di/AppModule);
// on iOS via a simple service locator.
class KingdomGenerator(
    private val cardDao: CardDataSource,
    private val expansionDao: ExpansionDataSource,
    private val userPrefsRepository: UserPrefsSource,
    ...
    suspend fun generateKingdom(): Kingdom {
        ...
        logI("Kingdom Generator", "Starting generation - Mode: ${config.randomMode} - Pool: ${context.portraitPool.size} portraits, ${context.landscapePool.size} landscapes")
        ...
        val sortType = try {
            KingdomSortType.valueOf(sortTypeName)
        } catch (e: Exception) {
            KingdomSortType.EXPANSION
        }
    ...
    private fun sortKingdom(kingdom: Kingdom, sortType: KingdomSortType): Kingdom {
        return kingdom.copy(
    ...
    private fun sortCardMap(cards: LinkedHashMap<Card, Int>, sortType: KingdomSortType): LinkedHashMap<Card, Int> {
        ...
        val sorted = when (sortType) {
            ...
            KingdomSortType.COST -> cards.entries.sortedBy { it.key.cost ?: Int.MAX_VALUE }
        }
