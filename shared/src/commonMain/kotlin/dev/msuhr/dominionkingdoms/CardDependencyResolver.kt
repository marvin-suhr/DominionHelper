        ...
        when (prosperityMode) {
            ...
            ProsperityMode.TEN_PERCENT_PER_CARD -> {
                if (prosperityCount > 0) {
                    if (isPercentChance(minOf(prosperityCount * 10.0, 100.0))) {
                        logI(
                            "KingdomGenerator",
    ...
    private suspend fun getStartingCards(randomCards: kotlin.collections.Set<Card>): Map<String, Int> {
        ...
        when (darkAgesMode) {
            ...
            DarkAgesMode.TEN_PERCENT_PER_CARD -> {
                if (isPercentChance(minOf(darkAgesCount * 10.0, 100.0))) {
                    logI("KingdomGenerator", "Adding Shelters - 10% per card ($darkAgesCount) triggered")
        ...
        CardNames.heirloomPairs.forEach { (cardName, dependentCardName) ->
            if (randomCards.any { it.name == cardName }) {
                logD("Kingdom Generator", "Adding Heirloom: $cardName")
